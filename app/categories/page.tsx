...
export default function SpacesDirectory() {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-5 items-start">
      ...
      {/* Sidebar Topics */}
      <aside className="hidden md:block col-span-1 bg-white border border-[#e2e8f0] rounded-md p-3 text-xs">
        ...
        <div className="flex flex-col gap-1.5 font-medium text-gray-700">
          {highTraffic.map((t, idx) => (
            <a key={idx} href="#" className="hover:bg-gray-50 px-2 py-1 rounded transition hover:text-[#1d4ed8]">• {t}</a>
          ))}
        </div>
      </aside>
      ...
      <main className="col-span-1 md:col-span-3 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {categories.map((cat) => (
            <div key={cat.id} className="bg-white border border-[#e2e8f0] rounded-md p-4 shadow-xs hover:shadow-sm transition flex flex-col justify-between">
              <div>
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-xl">{cat.icon}</span>
                  <h2 className="text-sm font-bold text-gray-950 hover:text-[#1d4ed8] cursor-pointer transition">{cat.title}</h2>
                </div>
                <p className="text-xs text-gray-500 leading-relaxed mb-3">{cat.desc}</p>
                <div className="flex flex-wrap gap-1">
                  {cat.subs.map((s, i) => (
                    <span key={i} className="text-[10px] font-medium bg-blue-50 text-[#1d4ed8] px-1.5 py-0.5 rounded">
                      {s}
                    </span>
                  ))}
                </div>
              ...
              <div className="mt-4 pt-2 border-t border-gray-50 flex justify-end">
                <button className="text-xs font-semibold text-[#1d4ed8] hover:underline">
                  Follow Space
                </button>
              </div>
            </div>
